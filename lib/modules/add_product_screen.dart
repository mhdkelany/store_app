...
var formKey=GlobalKey<FormState>();
      ...
      listener: (context,state)
      {
        ...
        if(state is InsertProductForUserSuccessState) {
          ScaffoldMessenger.of(context).showSnackBar(buildSnackBar(
              Text('تمت اضافة المنتج'), Colors.greenAccent,
              Duration(seconds: 3)));
        ...
        if(state is InsertProductForUserErrorState)
          {
            ScaffoldMessenger.of(context).showSnackBar(buildSnackBar(Text('لم تتم اضافة المنتج'), Colors.red, Duration(seconds: 3)));
          }
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        DropdownButtonFormField<dynamic>(
                            items:StoreAppCubit.get(context).categoriesModel!=null? StoreAppCubit.get(context).categoriesModel!.data.map((e) =>DropdownMenuItem(child: Text('${e.name}'),value:e.idCate ,)).toList():[].map((e) => DropdownMenuItem(child: Text('$e'),value: e,)).toList(),
                            onChanged: (value)
                            {
                        ...
                        //                       {
                        //                         //print(item.toString().split(' Color(').last.substring(0,item.toString().split(' Color(').last.length-2));
                        //                         print(item);
                        //                       }
                        ...
                        ConditionalBuilder(
                          ...
                          builder: (context)=>  defaultButton(
                              function: state is InsertProductForUserLoadingState?null:()
                              {
                                if(formKey.currentState!.validate())
                                {
                                  if(StoreAppCubit.get(context).productImage!=null)
                                  {
                                    ...
                                    //  String convertBase64=base64Encode(StoreAppCubit.get(context).productImage!.readAsBytesSync());
                                      ...
                                      StoreAppCubit.get(context).insertProduct(data:
                                      {
                                      ...
                                      },path: StoreAppCubit.get(context).productImage!);
                                    }
                                    else
                                    {
                                      ScaffoldMessenger.of(context).showSnackBar(buildSnackBar(Text('يجب ان تختار قسم لهذا المنتج'), Colors.red, Duration(seconds: 2)));
                                    }
                                  }
                                  else{
                                    ScaffoldMessenger.of(context).showSnackBar(buildSnackBar(Text('يجب ان تختار صورة للمنتج'), Colors.red, Duration(seconds: 2)));
                                  }
                                }
                              },
  ...
  Colors.red,
      ...
      {
        print(color.toString().split(' Color(').last.substring(0,color.toString().split(' Color(').last.length-2));
        colors.add(color.toString().split(' Color(').last.substring(0,color.toString().split(' Color(').last.length-2));
